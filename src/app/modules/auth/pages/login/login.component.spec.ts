...
describe('LoginComponent', () => {
  ...
  beforeEach(async () => {
    component = new LoginComponent(router, toastr, store, actions);
  });
  ...
  test('should store dispatch LoginAction', () => {
    jest.spyOn(store, 'dispatch');
    store.dispatch(new Login({ email: '', password: '' }));

    expect(store.dispatch).toHaveBeenCalledWith(new Login({ email: '', password: '' }));
  });
});
